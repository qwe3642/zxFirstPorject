...
(function($) {
    //保存源方法
    $.fn.combogrid.methods_old = new Object();
    $.fn.combogrid.methods_old = $.fn.extend(true,{},$.fn.combogrid.methods);
    //改写方法
    /*
    ...
    */
        ...
        loadRefDataAfterProc:function(op,data){
            ...
            if (op.childOption.refId){
                ...
                if (c_op.refType == '0'){
                    //默认combobox组件
                    $(c_id).combogrid(c_op);
                    op.onSelect = function(rowh){
                    	var rec = $(p_id).combogrid('getRow',rowh);
				...
				if (op.isSetdefaultValue){
					var rec = $(p_id).combogrid('valueObj');
					if (rec) op.onSelect(rec.rowh);
				}
            }
        },
        childOption:{}
    ...
    $.extend($.fn.combogrid.methods,{
        ...
        callOldMethod:function(jq,key,a,b,c,d){
            var oldLoadDataMethod = $.fn.combogrid.methods_old[key]
            return oldLoadDataMethod.call($.fn.combogrid.methods,jq,a,b,c,d);
        },
        /**
         * 添加查询参数
         * @param params 参数
        ...
        valueObj:function(jq){
            ...
            var op = $(id).combogrid('options');
            var obj = new Object();
            var code = $(id).combogrid('getValue');
        ...
        childValueObj:function(jq){
        	...
        	return $(c_id).combogrid('valueObj');
        },
        /**
         * 获取子组件选中的值
         */
        getChildValue:function(jg){
        	var c_id = this.getChildRefId(jq);
            return $(c_id).combogrid('getValue');
        },
        /**
         * 获取子组件选中的文本
         */
